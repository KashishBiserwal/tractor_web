$(document).ready(function() {
   ...
   $('#VerifytreactorAndImplement').click(verifyotp3);
    ...
    function store(event) {
        ...
        // Check if image files are selected before proceeding
        var imageFilesArray = [];
        ...
        function submitForm() {
            ...
            $.ajax({
                url: 'https://shopninja.in/bharatagri/api/public/api/customer/customer_enquiries',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
...
function get_otp2(phone) {
    ...
    $.ajax({
        ...
        success: function (result) {
            console.log(result, "result");
            $('#myModal').modal("hide"); // Show OTP modal only after validation
        },
        error: function (error) {
            console.error('Error fetching data:', error);
        }
    ...
    $.ajax({
        ...
        error: function (xhr, textStatus, errorThrown) {
            ...
            if (xhr.status === 401) {
                console.log('Invalid credentials');
                var htmlcontent = `<p>Invalid credentials!</p>`;
                document.getElementById("error_message1").innerHTML = htmlcontent;
            } else if (xhr.status === 403) {
                console.log('Forbidden: You don\'t have permission to access this resource.');
                var htmlcontent = ` <p> You don't have permission to access this resource.</p>`;
                document.getElementById("error_message1").innerHTML = htmlcontent;
        ...
        $.ajax({
            ...
            error: function (xhr, textStatus, errorThrown) {
                ...
                if (xhr.status === 401) {
                    console.log('Invalid credentials');
                    var htmlcontent = `<p>Invalid credentials!</p>`;
                    document.getElementById("error_message2").innerHTML = htmlcontent;
                } else if (xhr.status === 403) {
                    console.log('Forbidden: You don\'t have permission to access this resource.');
                    var htmlcontent = ` <p> You don't have permission to access this resource.</p>`;
                    document.getElementById("error_message2").innerHTML = htmlcontent;
