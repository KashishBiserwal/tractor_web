                                                 ...
                                                 <div class="content d-flex flex-column flex-grow-1">
                                                     <div class="caption text-center">
                                                         <a href="detail_tractor.php?product_id=${p.product_id}" class="text-decoration-none text-dark">
                                                             <p class="pt-3"><strong class="series_tractor_strong text-center h6 text-truncate fw-bold ">${p.model}</strong></p>
                                                         </a>      
                                                     </div>
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.customerData && response.customerData.length > 0) {
                ...
                if (isUserLoggedIn()) {
                    $('#' + formId + ' input, #' + formId + ' select').not('#price_form,#district,#Tehsil').prop('disabled', true);
                }
            }
        },
        error: function(error) {
        ...
        success: function (searchData) {
            ...
            if (Array.isArray(allProductData)) {
                allProductData.forEach(function(product) {
                    ...
                    console.log(product);
                    appendFilterCard(filterContainer, product); 
                });
            }
        },
        error: function (error) {
            ...
            $("#load_moretract").hide();
            $("#noDataMessage").show(); // Show the message
            $("#noDataMessage img").attr("src", "assets/images/404.gif"); // Show the image
        },
    });
}
